...
public class ListaDeEnterosEnlazada extends ListaDeEnteros {
	...
	private int buscarNum(int minAnt) {
		int min = 999999999;
		this.comenzar();
		while (!this.fin()) {
			int act = this.proximo();
			if ((act < min) && (act > minAnt)) {
				min = act;
			}
		}
		return min;
	...
	public ListaDeEnterosEnlazada ordenar() {
		...
		resultado.comenzar();
		int minAnt = -1;
		while (this.tamanio() != resultado.tamanio()) {
			minAnt = buscarNum(minAnt);
			resultado.agregarFinal(minAnt);
		}
		return resultado;
	}
	// Recorre n veces los n elementos de la lista --> n cuadrado
	...
	public ListaDeEnterosEnlazada combinarOrdenado(ListaDeEnterosEnlazada listaParam) {
		...
		//Casos
		if (this.esVacia()) {
			resultado = listaParam;
		} else if (listaParam.esVacia()) {
			resultado = this;
		} else {
			
			while (!(this.fin() || listaParam.fin())) {
				if (this.actual.getDato() < listaParam.actual.getDato()) {
					resultado.agregarFinal(this.actual.getDato());
					this.proximo();
				} else {
					resultado.agregarFinal(listaParam.actual.getDato());
					listaParam.proximo();
				}
			}
			
			//Pongo en resultado lo que resta
			while (!(this.fin())) {
				resultado.agregarFinal(this.actual.getDato());
				this.proximo();
			}
