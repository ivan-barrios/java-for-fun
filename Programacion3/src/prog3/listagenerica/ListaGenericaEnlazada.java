...
public class ListaGenericaEnlazada<T> extends ListaGenerica<T> {
	...
	public ListaGenericaEnlazada<T> invertir() {
		ListaGenericaEnlazada<T> listaInvertida = new ListaGenericaEnlazada<T>();
		for (int i = 0; i < this.tamanio; i++) {
			listaInvertida.agregarFinal(this.elemento(i));
		}
		listaInvertida.hacerInversion();
		return listaInvertida;
	}
	
	
	public ListaGenericaEnlazada<T> hacerInversion() {
		if (!(tamanio <= 1)) {
			T primero = this.elemento(0);
			T ultimo = this.elemento(tamanio-1);
			this.eliminarEn(0);
			this.eliminarEn(tamanio-1);
			this.hacerInversion();
			this.agregarInicio(ultimo);
			this.agregarFinal(primero);
		}
		return this;
	}
